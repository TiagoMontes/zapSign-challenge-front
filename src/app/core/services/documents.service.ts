...
export class DocumentsService extends BaseApiService {
  ...
  isAnalysisInProgress(id: number): Observable<boolean> {
    return this.analysisLoading$.pipe(map((loadingSet) => loadingSet.has(id)));
  }
  ...
  getDocumentsCount(companyId?: number): Observable<number> {
    const relevantDocuments$ = companyId ? this.getDocumentsByCompany(companyId) : this.documents$;

    return relevantDocuments$.pipe(map((documents: Document[]) => documents.length));
  }
